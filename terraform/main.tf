...
resource "null_resource" "server-provisioner" {
  ...
  count = length(virtualbox_vm.server)
  connection {
    type = "ssh"
    host = element(
      virtualbox_vm.server.*.network_adapter.0.ipv4_address,
      count.index,
    )
    user     = "packer"
    password = "packer"
...
resource "null_resource" "client-provisioner" {
  ...
  count = length(virtualbox_vm.client)
  connection {
    type = "ssh"
    host = element(
      virtualbox_vm.client.*.network_adapter.0.ipv4_address,
      count.index,
    )
    user     = "packer"
    password = "packer"
